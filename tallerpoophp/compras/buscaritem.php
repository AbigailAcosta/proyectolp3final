...
if ($action == 'ajax') {
    ...
    $count_query = $articulos->getArticulos($sql);
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = 'agregarfactura.php';
    
    $sFields = "`articulos`.`id_Articulo`, `articulos`.`descripcion`, round(`articulos`.`preciocompra`, 0) as preciocompra, round(`articulos`.`precioventa`, 0) as precioventa";
    $sql = 'SELECT ' . $sFields . ' FROM ' . $sTable . $sWhere . ' LIMIT ' . $offset . ',' . $per_page;
    $query = $articulos->getArticulos($sql);
