...
if ($action == 'ajax') {
    ...
    $count_query = $articulos->getArticulos($count_sql);  // Pasa la consulta de conteo
    ...
    $sFields = "`articulos`.`id_Articulo`, `articulos`.`descripcion`, 
                ROUND(`articulos`.`precioventa`, 0) AS precioventa";
    $sql = "SELECT $sFields FROM $sTable $sWhere LIMIT $offset, $per_page";
    $query = $articulos->getArticulos($sql);  // Pasa la consulta para obtener los artículos
